...
export default function CurrentAuction() {
  ...
  function closeAuction() {
    ...
    if (!isEmpty(currentAuction)) {
      axios
        .patch(
          `${process.env.REACT_APP_API_URL}api/auction/close/${currentAuction._id}`
        )
  ...
  function getAuctions() {
    axios
      ...
      .then((res) => {
        ...
        if (
          res !== undefined &&
          res.data.currentAuction !== undefined &&
          moment(res.data.currentAuction.end)
            .add((res.data.currentAuction.catalogue.length - 1) * 5, "m")
            .isBefore(moment())
        ) {
            ...
            <div>
              <div
                style={{
                  ...
                  borderRadius: "20px",
                  padding: "5%",
                  color: "rgb(239, 193, 109)",
                }}
              >
                <h2 style={{ fontSize: "3rem", display: "flex", gap: "20px" }}>
                  {" "}
                  Titre :{" "}
                ...
                <br />
                <h2
                  style={{ fontSize: "2.5rem", display: "flex", gap: "20px" }}
                >
                  {" "}
                  Sous-titre :{" "}
                ...
                <br />
                <h2
                  style={{ fontSize: "1.8rem", display: "flex", gap: "20px" }}
                >
                  {" "}
                  Début :{" "}
                  <p style={{ fontSize: "1.6rem", color: "white" }}>
                    {moment(currentAuction.start).format("LLL")}
                  </p>
                </h2>
                <br />
                <h2
                ...
                >
                  ...
                  Fin :{" "}
                  <p style={{ fontSize: "1.6rem", color: "white" }}>
                    {moment(currentAuction.end).format("LLL")} (dernier lot :{" "}
                    {moment(currentAuction.end)
                      .add((currentAuction.catalogue.length - 1) * 5, "m")
                      .format("LLL")}
                    )
                  </p>
                </h2>
                      ...
                      <Accordion expanded={true}>
                        ...
                        >
                          ...
                          <Typography
                            sx={{ color: "text.secondary", flex: "1" }}
                          >
                            {lot.bids.length} enchérissements
                          </Typography>
                        </AccordionSummary>
                        <AccordionDetails>
                          {!isEmpty(lot.bids) &&
...
function BidCard(props) {
  ...
  function getUser() {
    axios
      .get(`${process.env.REACT_APP_API_URL}api/user/${bid.bidderId}`)
      .then((res) => {
        const user = res.data;
        setUser(user);
      })
  ...
  async function deleteBid() {
    ...
    await axios
      .delete(`${process.env.REACT_APP_API_URL}api/bid/${bid._id}`, bid)
    ...
    <div style={{ width: "100%" }}>
      {user !== undefined && (
        <TableRow sx={{ width: "100%", display: "flex" }}>
          <TableCell sx={{ flex: "6", padding: "5px 2px" }} align="left">
            <strong>
          ...
          <TableCell sx={{ flex: "4", padding: "5px 2px" }} align="left">
            {" "}
            {user.phoneNumber}
          </TableCell>
          <TableCell sx={{ flex: "4", padding: "5px 2px" }} align="right">
            <strong>{bid.amount} €</strong>
          </TableCell>
          <TableCell sx={{ flex: "4", padding: "5px 2px" }} align="right">
            {moment(bid.createdAt).format("LLL")}{" "}
          </TableCell>
          <TableCell sx={{ flex: "1", padding: "5px 2px" }} align="right">
            {isLast && (
              <DeleteIcon
                className="delete"
      ...
      {openConfirmDialog && (
        <ConfirmDialog
          message={"supprimer cette enchère"}
          open={openConfirmDialog}
          onClose={() => setOpenConfirmDialog(false)}
          yesFunction={deleteBid}
        />
      )}
    </div>
  );
